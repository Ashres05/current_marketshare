...
CREATE OR REPLACE TEMPORARY TABLE tmp_marketshare_forecasts_source AS
WITH dates AS (
        SELECT
...
DELETE FROM CURRENT_DEV.DATA.MARKETSHARE_FORECASTS AS target
WHERE NOT EXISTS (
    SELECT 1
    FROM tmp_marketshare_forecasts_source AS source
...
USING tmp_marketshare_forecasts_source AS source
